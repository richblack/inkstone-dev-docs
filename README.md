...
> **注意**：如果你正在開發中，也可以使用 SSE URL：`https://dev.finally.click/mcp/sse`

### 2. Cursor 配置
在 `Cursor Settings > MCP` 中新增：
- **Name**: `u6u`
- **Type**: `sse`
- **Value**: `https://dev.finally.click/mcp/sse` (Headers: `X-Api-Key: YOUR_API_KEY`)
...
- **動態 JSON 目錄**: [https://dev.finally.click/mcp/tools/catalog](https://dev.finally.click/mcp/tools/catalog) — *永遠與代碼 100% 同步。*
- **互動式測試界面**: [透過 dev.finally.click 啟動 MCP Inspector](https://dev.finally.click/mcp-inspector?url=https://dev.finally.click/mcp/sse) — *在瀏覽器直接呼叫工具。*
